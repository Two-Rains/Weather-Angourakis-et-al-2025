...
weather_model <- initialize_weather_model(seed = SEED, year_length = YEAR_LENGTH)
```
...
extract_params <- function(pars, group_name = NULL) {
  if (is.atomic(pars)) {
    return(list(names = group_name, values = pars))
  }
  
  names <- character(length(pars))
  values <- vector("list", length(pars))
  
  for (i in seq_along(pars)) {
    names[i] <- if (is.null(group_name)) names(pars)[i] else paste(group_name, names(pars)[i], sep = " - ")
    values[[i]] <- pars[[i]]
  }
...
initial_params <- list(
  ...
  values = unlist(weather_model$PARS[1:4])
)

# Extract remaining parameters
remaining_params <- lapply(names(weather_model$PARS)[5:length(weather_model$PARS)], 
                           function(name) extract_params(weather_model$PARS[[name]], name))
...
# Create the table
par_values <- cbind(all_params$names, all_params$values)
knitr::kable(par_values, 
             format = "html",
...
# Main execution
plot_name <- file.path(output_dir, paste0("FigX-weather_modelExample.", plot_file_format))
